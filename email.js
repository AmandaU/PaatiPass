...
exports.sendEmailConfirmation = functions.database.ref('/tickets/{userid}').onWrite((change) => {
  ...
  var imageData = qr.imageSync(val.reference, { type: 'png' });
  ...
  const message3 = 'Please present the attached code at the venue door';
  ...
  mailOptions.subject = 'JaylaTickets Order confirmation, reference number: ' + val.reference;
      mailOptions.html = '<strong>' + val.eventname +'</strong><br><strong>' + reference +'</strong></br><p>' + message1 +'</p><p>' + message2 +'</p><p>' + message3 +'</p><p>' + from +'</p>';
